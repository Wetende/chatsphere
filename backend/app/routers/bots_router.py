...
@router.get("/", response_model=BotList, dependencies=[Depends(rate_limit_user)])
...
@router.post("/", response_model=BotResponse, status_code=status.HTTP_201_CREATED, dependencies=[Depends(rate_limit_user)])
...
@router.get("/{bot_id}", response_model=BotResponse, dependencies=[Depends(rate_limit_user)])
...
@router.put("/{bot_id}", response_model=BotResponse, dependencies=[Depends(rate_limit_user)])
...
@router.delete("/{bot_id}", status_code=status.HTTP_204_NO_CONTENT, dependencies=[Depends(rate_limit_user)])
